...
namespace Pathfinding {
	...
	public class AILerp : VersionedMonoBehaviour, IAstarAI {
		    ...
		    // These are already calculated in the MovementUpdate method
		public float horizontalSpeed { get; private set; }
		public float verticalSpeed { get; private set; }

		// If you want to make it accessible to other scripts
		...
		public void MovementUpdate (float deltaTime, out Vector3 nextPosition, out Quaternion nextRotation) {
			...
			Vector3 movementDelta = nextPosition - simulatedPosition;
			
			if (orientation == OrientationMode.ZAxisForward) {
				// 3D mode: horizontal is XZ plane, vertical is Y
				horizontalSpeed = new Vector3(movementDelta.x, 0, movementDelta.z).magnitude / deltaTime;
				verticalSpeed = movementDelta.y / deltaTime;
			} else {
				// 2D mode: horizontal is XY plane, vertical is Z
				horizontalSpeed = new Vector3(movementDelta.x, movementDelta.y, 0).magnitude / deltaTime;
				verticalSpeed = movementDelta.z / deltaTime;
			}
